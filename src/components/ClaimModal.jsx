...
export default function ClaimModal({ onClose }) {
  ...
  const toggleImpact = (item) => {
    setImpacts(prev => prev.includes(item) ? prev.filter(i => i !== item) : [...prev, item]);
  };
      ...
      <div className="relative w-full max-w-lg bg-white shadow-2xl rounded-2xl overflow-hidden">
        ...
        <div className="px-6 pt-6 pb-4 border-b border-[#e4e4e7]">
          <div className="relative flex flex-col items-center mb-3">
            <button onClick={onClose} className="absolute right-0 top-0 text-[#71717a] hover:text-[#3a3a3a] transition-colors">
              <X size={20} />
            </button>
            <h3 className="text-xl font-black uppercase tracking-tight text-center" style={{ fontFamily: "'Playfair Display', serif" }}>
              Tell Us What Happened
            </h3>
            {!submitted && (
              <p className="text-xs text-[#71717a] mt-0.5 text-center">You don't have to figure this out alone. We'll walk you through it.</p>
            )}
          </div>
          {!submitted && (
        ...
        <div className="p-6 min-h-[320px] flex flex-col justify-between items-center text-center">
          {submitted ? (
            <div className="flex flex-col items-center justify-center text-center space-y-4 py-8">
              ...
              <div className="space-y-2">
                <h4 className="text-2xl font-black" style={{ fontFamily: "'Playfair Display', serif" }}>
                  {name ? `${name}, we've heard you` : "We've heard you"}
                </h4>
                <p className="text-sm font-black text-[#c5a043] uppercase tracking-widest">and we're here to help.</p>
            ...
            <>
              ...
              {step === 0 && (
                <div className="space-y-4 w-full flex flex-col items-center justify-center pt-8">
                  <p className="text-lg font-black" style={{ fontFamily: "'Playfair Display', serif" }}>First, what's your name?</p>
                  <input
                    autoFocus
                    type="text"
                    value={name}
                    ...
                    {INCIDENT_TYPES.map(({ label, img }) => (
                      <button
                        ...
                        className={`relative overflow-hidden rounded-xl aspect-square flex flex-col items-center justify-end p-2 transition-all border-2 ${
                          incident === label ? 'border-[#c5a043] scale-[1.04]' : 'border-transparent'
                        }`}
                      >
                        <img src={img} alt={label} className="absolute inset-0 w-full h-full object-cover" />
                        <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent" />
                        <span className="relative z-10 text-white text-[9px] font-black uppercase tracking-tight text-center leading-tight">{label}</span>
                        {incident === label && (
              ...
              {step === 2 && (
                <div className="space-y-4">
                  <p className="text-lg font-black" style={{ fontFamily: "'Playfair Display', serif" }}>Can you tell me what happened?</p>
                  <textarea
                    autoFocus
                    value={story}
              ...
              {step === 3 && (
                <div className="space-y-4">
                  <p className="text-lg font-black" style={{ fontFamily: "'Playfair Display', serif" }}>How has this affected you?</p>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {IMPACTS.map(item => (
                      ...
                      >
                        {impacts.includes(item) && <Check size={12} className="mb-1 text-[#c5a043]" />}
                        {item}
                      </button>
                    ))}
                  </div>
              ...
              {step === 4 && (
                <div className="flex flex-col items-center justify-center text-center space-y-4 py-6">
                  ...
                  <p className="text-sm text-[#71717a] max-w-xs">This is really helpful. Just a couple more questions and we'll have everything we need.</p>
                </div>
              )}
              ...
              {step === 5 && (
                <div className="space-y-4">
                  <p className="text-lg font-black" style={{ fontFamily: "'Playfair Display', serif" }}>Have you spoken to a lawyer yet?</p>
              ...
              {step === 6 && (
                <div className="space-y-4">
                  <p className="text-lg font-black" style={{ fontFamily: "'Playfair Display', serif" }}>
                    {name ? `${name}, where should we send your case review?` : 'Where should we send your case review?'}
                  </p>
                  <input
                    type="tel"
                    value={phone}
          ...
          {!submitted && (
            <div className="flex items-center justify-between mt-6 pt-4 border-t border-[#e4e4e7] gap-6">
              {step > 0 ? (
                <button onClick={back} className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-[#71717a] hover:text-[#3a3a3a] transition-colors px-4 py-3">
              ...
              {step < TOTAL_STEPS - 1 ? (
                ...
                <button
                  ...
                  className="flex items-center gap-2 bg-[#c5a043] text-white px-8 py-3 rounded-xl font-black uppercase tracking-widest text-xs hover:opacity-90 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  Submit My Case <ChevronRight size={14} />
                </button>
              )}
            </div>
          )}
