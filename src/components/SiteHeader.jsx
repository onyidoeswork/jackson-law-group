    ...
    >
      ...
      {mobileOpen && (
        <div className="absolute top-full left-0 right-0 bg-white border-t border-[#e4e4e7] shadow-lg lg:hidden z-50 p-6 flex flex-col gap-4">
          ...
          <button onClick={() => { onOpenModal(); setMobileOpen(false); }} className="bg-[#c5a043] text-white px-6 py-3 rounded-sm font-bold uppercase tracking-widest text-sm">Free Case Review</button>
        </div>
      )}
    </header>
  );
